...
class GeneticAlgorithm:
            ...
            while (killer.kill_now == False and ((time.time() - watchStart) * 1000) < millisecondsToProcess):
                ...
                for i in range(0,maxBreedingPerGeneration):
                    ...
                    if (i == 0):
                        ...
                        a = int(localEaseOutExpo (0, 1, localPRNG.random()) * localNumberOfOrganisms)
                        b = int(localEaseOutExpo (0, 1, localPRNG.random()) * localNumberOfOrganisms)
                        ...
                        a = int(localEaseInExpo (0, 1, localPRNG.random()) * localNumberOfOrganisms)
                        b = int(localEaseOutExpo (0, 1, localPRNG.random()) * localNumberOfOrganisms)
                        breedOrganisms (allOrganisms [a], allOrganisms [b], newChild, localPRNG)
                    elif (i == 2):
    ...
    def PerformGeneticsThreaded (self, millisecondsToProcess, nCPU=-1):
        ...
        return (self.masterBestOrganism,self.masterBestOrganismScore)
        
    def _WorkerProcess(self, randomSeed,millisecondsToProcess,sharedOrganismIdx,numThreads):
        
        self.prng.seed(randomSeed)
        
        bestOrganism = self._PerformGenetics (millisecondsToProcess, self.generateOrganism, self.breedOrganisms, self.scoreOrganism, self._CaptureMasterGenerationChosenPassthrough, sharedOrganismIdx, (sharedOrganismIdx + 1) % numThreads)
